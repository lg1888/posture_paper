...
\title{Artificial-Intelligence-Based Ergonomic Risk Detection and Recommendation Generation from Construction Video}
...
\begin{abstract}
Work-related musculoskeletal injury remains a significant concern in construction. This study presents an artificial-intelligence-based framework for ergonomic risk detection and recommendation generation from single-camera construction video. The system combines human pose estimation, temporal activity tracking, full-body Rapid Entire Body Assessment scoring, approximate load interpretation using a vision-language model, and retrieved text generation for context-aware safety recommendations. The framework was evaluated on videos of drywall finishing, hand screeding, kneeling work, concrete block laying, and tying rebar. Manual annotations were used to assess posture-duration and activity-count accuracy, and additional benchmarks compared the proposed pipeline against OpenPose and RTMPose on representative clips. The results show strong agreement with manual measurements, overall activity-classification accuracy above 90\% across the evaluated task categories, and a favorable tradeoff between temporal stability and runtime for the proposed smoothed pose-estimation pipeline. The system provides continuous posture monitoring, whole-body ergonomic risk scoring, and recommendation support that can help construction managers identify unsafe work patterns in ordinary site video.
\end{abstract}

\noindent \textbf{Keywords:} ergonomic risk assessment, construction safety, computer vision, human pose estimation, vision-language models, recommendation generation, artificial intelligence
...
Computer vision research has attempted to address these limitations by automating posture recognition from images and video. Earlier methods relied on 2D image features and conventional classifiers \citep{yan2017development,seo2021automated}. More recent approaches use deep learning, 3D pose estimation, and automated ergonomic scoring. For example, \citet{yu2019joint} integrated 3D pose estimation with REBA, \citet{massirisfernandez2020ergonomic} computed RULA scores from image data, and \citet{li2024data} combined 3D RGB-based pose estimation with a data-driven inference framework. Temporal modeling has also begun to appear in the literature through approaches such as recurrent models for posture recognition and transformer-based repetitive action counting \citep{zhao2021applying,wang20233d,chen2024automatic}. However, most prior studies still emphasize posture recognition or scoring at the frame level. In contrast, the present work combines pose estimation with temporal bending-frequency tracking, posture-duration measurement, full-body REBA scoring, vision-language-model-assisted approximate load estimation, and retrieval-augmented language-model feedback so that detected risks can be interpreted and acted upon within a single workflow.
...
\begin{subfigure}[b]{0.9\textwidth}
    \includegraphics[width=\textwidth]{figures/Figure1_Updated.pdf}
    \end{subfigure}
...
This module classifies worker activities using pose landmarks and evaluates ergonomic risk using the full REBA framework \citep{hignett2000rapid}. The system first detects body keypoints using YOLO pose and then computes posture angles for the neck, trunk, upper arms, lower arms, wrists, and legs from smoothed 2D keypoint coordinates. These segment angles are converted into REBA segment scores, combined through REBA Table A, Table B, and Table C, adjusted with a load/force score, and finally mapped to the corresponding REBA risk level. In this way, the reported metric is a full REBA score rather than a trunk-only proxy.
...
\begin{equation}
    \theta = \arccos \Bigl( \frac{\vec{u}\cdot\vec{v}}{\|\vec{u}\| \|\vec{v}\|} \Bigr)\times \frac{180}{\pi}.
\end{equation}
...
This module generates individualized ergonomic risk recommendations through a two-stage pipeline that uses both a vision-language model and a large language model. Figure~\ref{fig3r} shows the overall framework. First, the system assembles the inputs needed for recommendation. A vision-language model such as Qwen2 or Molmo \citep{deitke2024molmo} produces a text description of the construction activity from an input image, including salient actions, tools, and observable context. An example prompt is provided in Table~\ref{table:prompts}. Posture statistics are obtained from the YOLOv8 method described earlier, including bending counts and bending duration for each individual. These elements are programmatically inserted into a template, where placeholders enclosed in angle brackets represent content returned by the models.

Second, the system generates recommendations with retrieval-augmented generation. The implementation uses ChromaDB as the vector database, the \textit{all-MiniLM-L6-v2} model to embed text chunks, \textit{gpt-4-turbo} as the language model, and Streamlit as the user interface. The knowledge base consists of 126 embedded text chunks derived from \textit{Simple Solutions: Ergonomics for Construction Workers} \citep{albers2007simple}. Retrieval is performed with normalized cosine similarity, and only passages above a relevance threshold are forwarded to the generator. The language model then composes answers to the three targets specified in Table~\ref{table:prompts}: causes of risk, mitigation strategies, and corrective actions. The recommendations are grounded by the retrieved passages and are returned with supporting snippets for transparency.

In the present study, the quality and effectiveness of the generated recommendations were not formally evaluated through expert scoring, inter-rater agreement, user studies, or task-level outcome analysis. Accordingly, the vision-language and retrieval-augmented recommendation component should be interpreted as a proof-of-concept module whose outputs are grounded in retrieved ergonomic guidance, but whose practical effectiveness has not yet been systematically validated. A formal evaluation of recommendation quality remains an important direction for future work.
...
Figure \ref{fig5} shows skeletal overlays of workers performing drywall finishing tasks in four different scenarios. Drywall finishing involves the process of smoothing, taping, and sealing joints between drywall sheets after they are mounted on walls or ceilings. This work often requires individuals to bend and crouch, as they need to reach lower sections of the wall or work in confined spaces to apply compound and tape properly. Prolonged bending during drywall finishing can strain the lower back, which increases the risk of injury. Figure \ref{fig5(a)} shows a single worker labeled as ``Person 1,'' who remains standing across multiple frames. The skeletal overlays display straight lines that represent the worker's upright posture, and no red lines appear in these frames, which means that no bending was detected. Figure \ref{fig5(b)} includes two workers labeled as ``Person 1'' and ``Person 2.'' Each worker performs different activities over time. ``Person 1'' moves between normal and risky postures, while ``Person 2'' remains in a normal posture without major changes. Figure \ref{fig5(c)} illustrates the activities of ``Person 2'' across a sequence of frames, where the worker alternates between standing and bending. Figure \ref{fig5(d)} adds a crowded overlap scenario in the same green/red skeleton style. It highlights that the posture output remains visually informative, but overlapping workers make limb separation and person association more difficult.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Drywall Finishing Test 4.png}
        \caption{Scenario 4}
        \label{fig5(d)}
    \end{subfigure}
...
\toprule
\textbf{Task} & \textbf{Case} & \textbf{Model Bend Dur.} & \textbf{Manual Bend Dur.} & \textbf{Model Bend Cnt.} & \textbf{Manual Bend Cnt.} & \textbf{Error (\%)} \\
\midrule
...
To provide a direct quantitative comparison with established alternatives, we benchmarked three pose-estimation configurations on five representative clips from the construction dataset: OpenPose, RTMPose, and YOLOv8 with landmark smoothing. Each clip contained 10 sampled frames, and all methods were evaluated using the same downstream processing pipeline for keypoint filtering, segment-angle estimation, bending detection, and full-body REBA scoring. The benchmark reports required-keypoint visibility, required-joint coverage, mean and maximum REBA score, REBA standard deviation, mean trunk-angle jitter, proportion of bending frames, and runtime per frame.

Table \ref{table:benchmark_summary} summarizes the aggregate results. RTMPose achieved the highest required-keypoint visibility and joint coverage, at 54.0\% and 78.77\%, respectively, and also produced the highest mean REBA score of 5.5. However, it showed substantially larger temporal instability, with mean trunk jitter of 25.1$^\circ$, and required 390.53 ms/frame. YOLOv8 with smoothing achieved the lowest trunk jitter at 8.69$^\circ$ and the fastest runtime at 58.38 ms/frame, while still maintaining strong mean required-joint coverage of 71.75\%. OpenPose remained the weakest overall baseline, with only 46.16\% coverage, 4.0\% visibility, and 2829.41 ms/frame runtime. These results support the use of the smoothed YOLOv8 pipeline when temporal stability and near-real-time performance are primary requirements, while RTMPose serves as a strong higher-coverage comparison model.

  Figure \ref{fig:benchmark_cases_grid} presents all five benchmarked cases in a compact 3-column layout: OpenPose, RTMPose, and YOLOv8 with smoothing. Case 1 (drywall clip) shows that RTMPose improves required-keypoint visibility to 70.0\%, but YOLOv8 with smoothing delivers the highest required-joint coverage at 83.52\% and the lowest trunk jitter at 13.13$^\circ$, compared with 16.98$^\circ$ for RTMPose and 18.45$^\circ$ for OpenPose. Case 2 (hand1 clip) highlights the main tradeoff between RTMPose and the proposed model: RTMPose achieves the highest joint coverage at 89.23\%, whereas YOLOv8 with smoothing preserves the same 60\% bending-frame proportion with much lower trunk jitter, 10.08$^\circ$ versus 16.94$^\circ$. Case 3 (kneeling clip) shows the clearest temporal advantage of the proposed model, with YOLOv8 with smoothing reducing trunk jitter to 0.13$^\circ$, while RTMPose reaches 32.03$^\circ$ despite stronger coverage. Case 4 (block clip) shows full visibility and coverage for both RTMPose and YOLOv8 with smoothing, but the proposed model remains substantially more stable, with 11.43$^\circ$ jitter versus 24.55$^\circ$ for RTMPose. Case 5 (hand3 clip) is the most difficult example: RTMPose attains the highest coverage at 68.46\%, while YOLOv8 with smoothing provides lower jitter, 20.0$^\circ$ versus 35.0$^\circ$, and OpenPose remains the weakest baseline. We now benchmark RTMPose directly, but VideoPose3D and Poserac were not evaluated in the present study; they remain important future baselines for broader comparison, especially for 3D temporal pose refinement and repetitive-action modeling.
...
\midrule
OpenPose & 46.16 & 4.58 & 20.21 & 32.0 & 2829.41 \\
RTMPose & 78.77 & 5.50 & 25.10 & 50.0 & 390.53 \\
YOLOv8 + smoothing & 71.75 & 4.78 & 8.69 & 44.0 & 58.38 \\
\bottomrule
\end{tabular}
...
\begin{tabular}{c c c c}
 & \textbf{OpenPose} & \textbf{RTMPose} & \textbf{YOLOv8 + smoothing} \\
\textbf{\shortstack{Case 1\\Drywall}} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/drywall_clip__OpenPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/drywall_clip__RTMPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/drywall_clip__YOLOv8_plus_smoothing.png} \\
\textbf{\shortstack{Case 2\\Hand 1}} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/hand1_clip__OpenPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/hand1_clip__RTMPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/hand1_clip__YOLOv8_plus_smoothing.png} \\
\textbf{\shortstack{Case 3\\Kneeling}} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/kneeling_clip__OpenPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/kneeling_clip__RTMPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/kneeling_clip__YOLOv8_plus_smoothing.png} \\
\textbf{\shortstack{Case 4\\Block}} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/block_clip__OpenPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/block_clip__RTMPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/block_clip__YOLOv8_plus_smoothing.png} \\
\textbf{\shortstack{Case 5\\Hand 3}} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/hand3_clip__OpenPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/hand3_clip__RTMPose.png} &
\includegraphics[width=0.24\textheight,height=0.13\textwidth,keepaspectratio]{best_frames/hand3_clip__YOLOv8_plus_smoothing.png} \\
\end{tabular}
...
Figure~\ref{fig:molmo-result} shows the image of a worker tying rebar at a construction site, along with the prompt and the response generated by Molmo, a vision-language model \citep{deitke2024molmo}. The description emphasizes that the worker is adjusting, connecting, and securing thin metal rods. This detailed account helps the system understand the work being performed and serves as an essential input for subsequent ergonomic risk analysis and recommendation generation. The response from the retrieval-augmented generation module explains that workers can face risks such as poor posture, repeated bending, and awkward movements, all of which can lead to injury. It also offers practical suggestions, such as using improved tools and alternating tasks regularly.
...
The proposed real-time ergonomic risk detection system is able to identify and assess construction workers' postures from ordinary video. The results demonstrate the system's capability to track bending and standing postures, measure activity duration, count posture transitions, and compute a full-body REBA score from pose keypoints. The revised workflow evaluates neck, trunk, upper-limb, wrist, and leg postures, combines them through REBA Tables A, B, and C, and incorporates a vision-language-model-assisted approximate load/force estimate to assign the final risk level. Landmark smoothing improves temporal stability of the detected keypoints, and the benchmark shows that the YOLOv8-based pipeline provides the strongest balance of temporal stability and runtime efficiency across the five representative construction clips, while RTMPose offers a higher-coverage comparison baseline. By providing real-time feedback, the system enables safety managers to proactively identify unsafe ergonomic patterns. A notable advantage of the proposed system is its cost-effectiveness and accessibility. Unlike traditional ergonomic assessment methods that often require expensive specialized equipment, this system can be implemented using standard hardware, a commercial-grade camera, and a computer with moderate processing capabilities.
