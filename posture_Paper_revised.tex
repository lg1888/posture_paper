...
\noindent \textbf{Graphical Abstract:} \textit{[To be inserted: pipeline schematic from raw video input $\rightarrow$ YOLOv8 pose estimation $\rightarrow$ landmark smoothing $\rightarrow$ bending classification and REBA scoring $\rightarrow$ RAG-LLM recommendation output.]}
...
The ergonomic risk detection system is designed using the YOLOv8 architecture \citep{redmon2016you, terven2023comprehensive}. The system begins with an input image resized to 640×640×3 and processes it through three primary stages: backbone, neck, and head. In the context of the YOLOv8 network, these terms refer to standard stages of the detection architecture and do not refer to anatomical regions of the worker. The backbone extracts key features using CBS modules (convolution, batch normalization, and SiLU, the Sigmoid Linear Unit activation) to enhance essential patterns such as edges, shapes, and textures. To capture a wide range of details, C2f blocks \citep{wang2020cspnet} (a lightweight cross stage partial design that improves gradient flow and feature reuse) are applied. Stride 2 convolutions reduce the spatial size of the feature maps, and an SPPF block \citep{he2015spatial} (Spatial Pyramid Pooling Fast) at the end of the backbone aggregates multiscale context. Upsampling is performed later in the neck for multiscale feature fusion. 
The neck serves as the feature aggregation stage, where extracted features are refined and prepared for detection. It uses a feature pyramid with path aggregation, commonly implemented as an FPN \citep{lin2017feature} (Feature Pyramid Network) with PAN \citep{liu2018path} (Path Aggregation Network), to combine information from multiple scales. This enables the system to focus on both small and large details simultaneously. The neck also employs upsampling and concatenation operations, which preserve key details and create a clear and comprehensive feature map for further analysis. The head of the architecture is designed to detect body keypoints and poses. It includes three detection heads, each operating at a different spatial scale for small, medium, and large instances. Each head is anchor free and decoupled, producing a person bounding box, class scores, and K keypoints per instance, where K is the number of defined body keypoints. The higher resolution scale helps localize small structures such as hands and feet, the lower resolution scales capture larger structures such as the torso and hips, and the medium scale supports intermediate parts such as elbows and knees. A minimum confidence threshold of 0.5 was selected after experimenting with various values to balance detection accuracy and reliability. The present work uses only the keypoint outputs of the pre-trained \texttt{yolov8n-pose} model without modifying the internal network weights or architecture.
...
Pose estimation in this approach relies on YOLOv8 \citep{redmon2016yolo, terven2023comprehensive} to estimate body keypoints for worker posture assessment: head, shoulders, hips, knees, and ankles. The specific model used is 	exttt{yolov8n-pose}, a pre-trained lightweight variant. Figure \ref{fig2} represents the various keypoints of the human body that are identified by the YOLOv8 model. The model identifies these keypoints and makes connections between various poses to create a skeleton of the person detected.
...
The landmark smoothing scheme significantly improves the coherence of posture detections, particularly in challenging conditions where rapid movements may introduce noise in the detection process. A key source of instability addressed by this step is missing-data artefacts: when a keypoint is not detected with sufficient confidence, the model outputs a default coordinate of $(0,0)$ --- the image origin --- rather than a valid body position. Without smoothing, these spurious $(0,0)$ values are passed directly to the angle computation and produce erroneous bending detections; the smoothing interpolates over these frames and suppresses such artefacts. Enhancements were made to the existing YOLO model by integrating a smoothing technique which contributed to increasing the stability and coherence of pose estimation. The smoothing function averaged for every keypoint in position through successive frames. The formula used to calculate and incorporate the landmark smoothing mechanism is 
...
The smoothing factor reduces jitter in detected keypoints across frames by averaging the current frame’s coordinates with previous frames. It does not affect which detections are kept, but changes how detected keypoints are positioned and stabilized over time. A smoothing factor of 0.5 was chosen to achieve a balanced trade-off between responsiveness and stability in keypoint tracking. With $\alpha = 0.5$, the formula reduces to a simple arithmetic mean of the previous and current frame coordinates, giving equal weight to both. Lower values made the output overly sluggish, which caused delays in reflecting quick posture changes. Higher values introduced instability by allowing too much jitter from rapid movements. A value of 0.5 provided sufficiently smooth trajectories without sacrificing the system’s ability to capture real-time posture dynamics accurately. 

\subsection{Ergonomically Risky Activity Detection}
This module classifies worker activities using pose landmarks and evaluates ergonomic risk according to the REBA method \citep{hignett2000rapid}.

\textbf{Model selection rationale.} YOLOv8-pose was selected over alternative frameworks such as OpenPose \citep{cao2019openpose}, VideoPose3D \citep{pavllo20193d}, and RTMPose \citep{jiang2023rtmpose} for three reasons: (1) it is a single-stage detector that jointly localises persons and keypoints in one forward pass, enabling real-time edge deployment; (2) its pre-trained weights are validated for construction-site safety monitoring tasks \citep{kim2023application}; and (3) the \texttt{yolov8n-pose} variant runs at real-time frame rates on a standard laptop without a dedicated GPU. Integration of 3D lifting methods such as VideoPose3D is identified as a priority for future work.

\textbf{Person re-identification across frames.} To maintain consistent per-person activity metrics across frames, a centroid-based tracker is applied: the centroid of each detected bounding box is matched to the nearest centroid from the previous frame within a fixed pixel distance threshold, and a persistent person identifier is assigned accordingly. REBA provides standardized scoring to reflect the severity of bending and other postural loads. In this study, we focused on bending, with the trunk angle (i.e., forward flexion of the torso) serving as a key parameter in REBA scoring. The focus of this study was solely on trunk flexion because the primary objective was to determine ergonomic risk based on bending posture statistics of workers. Since bending frequency and duration are central to trunk flexion assessment, this study applies REBA scoring only to the trunk flexion component.
...
\noindent where \(\theta_{\text{trunk}}\) is measured in degrees. According to REBA, the trunk flexion score for bending is typically assigned as shown in Table~\ref{table:trunkscore}. Note that this table covers the trunk score component. The full REBA assessment is computed by combining all body segment scores as described in the subsection below. The final REBA score also considers other body segments (e.g., neck, legs, arms), coupling (e.g., load handling), and any twisting or lateral bending.
...
\textbf{Full-body REBA scoring.} In addition to the trunk angle, all joint angles are computed using the dot-product formula
\begin{equation}
    \theta = \arccos\!\left(\frac{\vec{u}\cdot\vec{v}}{\|\vec{u}\|\,\|\vec{v}\|}\right)\times\frac{180}{\pi}
\end{equation}
applied to pairs of smoothed 2D keypoint vectors. The six body-segment angles and their keypoint pairs are: trunk (hip-midpoint to shoulder-midpoint against vertical); neck (shoulder-to-ear against trunk axis); left and right upper arm (shoulder-to-elbow against trunk axis); left and right lower arm (elbow-to-wrist against upper arm); and knee flexion (hip-to-knee against knee-to-ankle). Each angle is mapped to its REBA segment score per the standard thresholds \citep{hignett2000rapid}: trunk score 1/2/3/4 for $<5^\circ$/$5$--$20^\circ$/$20$--$60^\circ$/$>60^\circ$; neck score 1/2 for $\leq20^\circ$/$>20^\circ$; upper arm score 1--4 per $<20^\circ$/$20$--$45^\circ$/$45$--$90^\circ$/$>90^\circ$; lower arm score 1 if $60^\circ$--$100^\circ$, else 2; wrist score defaulted to 1 (not directly observable from 2D skeleton); leg score based on stance with knee flexion modifiers. Score~A is looked up from REBA Table~A (trunk, neck, leg), Score~B from Table~B (upper arm, lower arm, wrist), and the final REBA score from Table~C, yielding a value in [1,15] mapped to Negligible/Low/Medium/High/Very High risk levels. Load, coupling, and activity modifiers are set to neutral defaults as these factors are not directly observable from image-based pose alone; the resulting scores should therefore be interpreted as conservative lower-bound estimates of the full REBA assessment.
...
In real time, the system measures the trunk angle, calculates the REBA trunk score, and updates bending duration and count. If a worker leaves the field of view, the current activity segment is closed at the last valid frame, and any reappearance is treated as a new tracking segment. These factors are used to estimate an overall REBA score or to trigger alerts if bending duration and count exceed recommended thresholds. When a worker is flagged for prolonged bending or frequent transitions into bending, further ergonomic analysis may be required to determine the overall REBA risk level and implement risk mitigation strategies.

\subsubsection{Automated Full-Body REBA Scoring}

To strengthen the ergonomic-risk interpretation, an automated full-body REBA scoring component is integrated into the overall workflow. For every frame, the smoothed \texttt{yolov8n-pose} skeleton is used to derive six joint-angle descriptors, which are then mapped through the standard REBA Table~A, Table~B, and Table~C lookup procedure \citep{hignett2000rapid} to obtain a final per-frame REBA score.

All joint angles are computed using the dot-product formula $\theta = \arccos\!\left(\vec{u}\cdot\vec{v}/\|\vec{u}\|\|\vec{v}\|\right)\times180/\pi$ applied to pairs of smoothed 2D keypoint vectors. The six body-segment angles are defined as follows:
\begin{itemize}
    \item \textbf{Trunk angle}: hip-midpoint-to-shoulder-midpoint vector against the vertical axis, measuring forward flexion.
    \item \textbf{Neck angle}: shoulder-to-ear vector against the trunk axis, measuring forward neck flexion.
    \item \textbf{Upper arm angles (left/right)}: shoulder-to-elbow vector against the trunk axis.
    \item \textbf{Lower arm angles (left/right)}: elbow-to-wrist vector against the upper arm vector.
    \item \textbf{Knee flexion angles (left/right)}: hip-to-knee vector against the knee-to-ankle vector.
\end{itemize}

Each angle is mapped to its REBA segment score per the standard thresholds \citep{hignett2000rapid}: trunk score 1/2/3/4 for $<5^\circ/5$--$20^\circ/20$--$60^\circ/>60^\circ$; neck score 1/2 for $\leq20^\circ/>20^\circ$; leg score 1 for bilateral stance with knee flexion modifiers; upper arm score 1--4 per $<20^\circ/20$--$45^\circ/45$--$90^\circ/>90^\circ$; lower arm score 1 if $60^\circ$--$100^\circ$, else 2; wrist score defaulted to 1 (not reliably observable from 2D skeleton alone). Score~A is looked up from REBA Table~A (trunk, neck, leg); Score~B from Table~B (upper arm, lower arm, wrist); and the final REBA score from Table~C, yielding a value in [1,15] mapped to Negligible/Low/Medium/High/Very High risk levels. Load, coupling, and activity modifiers are set to neutral defaults (0), as these factors are not directly observable from image-based pose alone --- an acknowledged limitation of the 2D approach.

\textbf{Model selection rationale.} Several alternative pose estimation frameworks exist, including OpenPose \citep{cao2019openpose}, VideoPose3D \citep{pavllo20193d}, and RTMPose \citep{jiang2023rtmpose}. YOLOv8-pose was selected for three practical reasons: (1) it is a single-stage detector that jointly localises persons and estimates their keypoints in one forward pass, enabling real-time deployment on edge devices; (2) its pre-trained weights are validated for construction-site safety monitoring tasks \citep{kim2023application}; and (3) the lightweight \texttt{yolov8n-pose} variant runs at real-time frame rates on a standard laptop without a dedicated GPU, making it accessible for small and medium construction projects. Integration of 3D lifting methods such as VideoPose3D is identified as a priority for future work.

\textbf{Person re-identification across frames.} To maintain consistent per-person activity metrics across frames, a centroid-based tracker is applied: the centroid of each detected bounding box is matched to the nearest centroid from the previous frame within a fixed pixel distance threshold, and a persistent person identifier is assigned. If a worker leaves the field of view, the current activity segment is closed at the last valid frame, and any reappearance is treated as a new tracking segment.
...
Second, the system generates recommendations with retrieval augmented generation (RAG). Relevant standards and public sources (e.g., \citep{albers2007simple}), including safety guidelines and incident statistics, form the knowledge base. The retrieval layer is implemented with ChromaDB, while sentence embeddings are generated with the \texttt{all-MiniLM-L6-v2} model. The local Chroma collection stores 126 embedded text chunks derived from the document \textit{Simple Solutions for Ergonomics in Construction} \citep{albers2007simple}. The RAG layer retrieves passages that match the current activity description and posture statistics using normalized cosine similarity with a relevance threshold. An LLM accessed through public APIs --- specifically \texttt{gpt-4-turbo} in the reference implementation --- then composes answers to three targets specified in Table~\ref{table:prompts}: causes of risk, mitigation strategies, and corrective actions. The recommendations are grounded by the retrieved passages and are returned with the supporting snippets for transparency. The current study has not yet performed a formal expert evaluation of recommendation quality; these outputs should be interpreted as pilot decision-support results rather than validated ergonomic prescriptions.
...
The landmark smoothing technique is used to enhance the system's ability to generate more refined and accurate representations of workers' postures during continuous movement. Figure \ref{fig4} illustrates the impact of landmark smoothing on pose estimation using YOLOv8. Figures \ref{fig:1b}, \ref{fig:2b}, and \ref{fig:3b} show keypoint positions without smoothing, where minor fluctuations between frames create inconsistencies. In the unsmoothed sequences, several keypoints appear at the image origin $(0,0)$: these are frames in which the detector confidence fell below the threshold and the keypoint was assigned a default coordinate of $(0,0)$ rather than being suppressed. When fed to the angle computation without smoothing, these null-coordinate frames produce spurious large trunk angles and erroneous bending detections. These variations can lead to inaccuracies in dynamic pose analysis, particularly in real-time applications. In contrast, Figures \ref{fig:a1}, \ref{fig:a2}, and \ref{fig:a3} present the results after applying landmark smoothing, which revealed more stable and consistent keypoint trajectories. The smoothing algorithm reduces abrupt changes by averaging each frame's landmark positions with those from previous frames by using a specified smoothing factor, effectively interpolating over missing-data frames and suppressing the $(0,0)$ artefact. This process stabilizes the skeletal representation, which ensures a more accurate depiction of movement over time. 
...
The current system presents a proof-of-concept for real-time ergonomic risk monitoring. Several limitations should be noted. First, the method relies on two-dimensional image coordinates, which may underestimate true joint angles when a worker rotates relative to the camera plane. Second, load, coupling, and wrist-twist factors in REBA are set to neutral defaults as they are not directly observable from the 2D skeleton; the reported scores are therefore conservative lower-bound estimates. Third, the RAG-LLM recommendation module has not yet been formally evaluated by domain experts; outputs should be treated as pilot decision-support results rather than validated ergonomic prescriptions.

Table~\ref{tab:comparison} places the proposed system alongside the most closely related methods in the literature. A direct quantitative comparison is not feasible because these methods were evaluated on different datasets and evaluation protocols; a controlled benchmark on shared construction-site data is identified as a priority for future work.

\begin{table}[H]
\centering
\small
\caption{Qualitative comparison with related pose-based ergonomic risk assessment methods}
\label{tab:comparison}
\begin{tabular}{p{3.2cm} p{2.0cm} p{1.5cm} p{1.6cm} p{1.7cm} p{2.0cm}}
\toprule
\textbf{Method} & \textbf{Pose input} & \textbf{Temporal} & \textbf{Full REBA} & \textbf{Auto feedback} & \textbf{Construction} \\
\midrule
Massiris Fern\'{a}ndez et al.\ \citep{massirisfernandez2020ergonomic} & 2D & No & Partial & No & No \\
Tao et al.\ \citep{tao2023postural} & 3D & No & Partial & Dashboard & No \\
Poserac \citep{yao2023poserac} & 2D & Yes & No & No & No \\
\textbf{Proposed} & \textbf{2D} & \textbf{Yes} & \textbf{Yes} & \textbf{RAG-LLM} & \textbf{Yes} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:reba_summary} reports the per-task REBA score summary computed from the per-frame automated scoring pipeline across the four pilot task categories.

\begin{table}[H]
\centering
\small
\caption{Per-task REBA score summary from the automated scoring pipeline}
\label{tab:reba_summary}
\begin{tabular}{lcccc}
\toprule
\textbf{Task} & \textbf{Workers} & \textbf{Mean REBA} & \textbf{Max REBA} & \textbf{High-risk frames (\%)} \\
\midrule
Drywall finishing     & 3 & 4.23 & 9  & 7.2  \\
Hand screeding        & 8 & 4.27 & 10 & 4.7  \\
Kneeling creepers     & 4 & 5.32 & 10 & 19.5 \\
Laying concrete block & 1 & 5.86 & 9  & 25.2 \\
\bottomrule
\end{tabular}
\end{table}

Kneeling-creeper and concrete-block tasks show the highest mean REBA and high-risk frame proportions, consistent with their sustained low-posture and heavy-lifting demands. The overall risk-level distribution across all frames was: Negligible 1.7\%, Low 13.6\%, Medium 66.5\%, High 18.0\%, and Very High 0.2\%.
...
Despite its effectiveness, the system has some limitations. The accuracy of pose detection may decrease in cases where workers' movements are partially occluded or when multiple workers overlap in the camera's field of view. Moreover, while the model can track posture transitions, it does not currently factor in external loads or other biomechanical stressors that contribute to ergonomic risks. The use of 2D joint angles may introduce inaccuracies particularly when the worker's torso is rotated relative to the camera plane; the reported REBA scores should be interpreted as conservative lower-bound estimates of the true ergonomic exposure, and integration of 3D lifting methods is identified as a priority for future work. The RAG-LLM recommendation module has not yet been formally evaluated by domain experts; outputs should be interpreted as pilot decision-support results rather than validated ergonomic prescriptions.

Table~\ref{tab:comparison} places the proposed system alongside the most closely related methods. A direct quantitative comparison is not feasible because these methods were evaluated on different datasets and task sets; a controlled benchmark on shared data is the primary priority for future work.
...
\caption{Qualitative comparison with related pose-based ergonomic risk assessment methods}
\label{tab:comparison}
\begin{tabular}{p{3.0cm} p{1.8cm} p{1.4cm} p{1.8cm} p{1.8cm} p{2.2cm}}
\toprule
\textbf{Method} & \textbf{Pose input} & \textbf{Temporal} & \textbf{Full-body REBA} & \textbf{Auto feedback} & \textbf{Construction domain} \\
\midrule
Massiris Fern\'{a}ndez et al.\ \citep{massirisfernandez2020ergonomic} & 2D & No & Partial & No & No \\
Tao et al.\ \citep{tao2023postural} & 3D & No & Partial & Dashboard & No \\
Chen and Yu \citep{chen2024automatic} & 2D & Yes & No & No & No \\
\textbf{Proposed (YOLOv8)} & \textbf{2D} & \textbf{Yes} & \textbf{Yes} & \textbf{RAG-LLM} & \textbf{Yes} \\
\bottomrule
\end{tabular}
\end{table}
